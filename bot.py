...
def get_today_totals(user_id):
    ...
    row = cur.fetchone()
    cur.execute("""
        SELECT COALESCE(SUM(amount),0) as water FROM water_log
...
async def show_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cur.execute("""
        ...
        WHERE user_id = %s AND DATE(logged_at) = %s ORDER BY logged_at
    """, (user_id, today))
    meals = cur.fetchall()
    cur.close()
    conn.close()
...
async def show_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    """, (user_id,))
    rows = cur.fetchall()
    cur.execute("SELECT calories_goal FROM users WHERE user_id = %s", (user_id,))
    user = cur.fetchone()
    cur.close()
    conn.close()
...
async def handle_water_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        conn = get_db()
        cur = conn.cursor()
        cur.execute("INSERT INTO water_log (user_id, amount) VALUES (%s, %s)", (user_id, amount))
        conn.commit()
        cur.close()
        conn.close()
        totals = get_today_totals(user_id)
